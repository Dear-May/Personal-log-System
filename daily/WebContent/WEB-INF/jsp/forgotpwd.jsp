...
<body>
	<div>
		...
		<form action="${pageContext.request.contextPath}/chang/paw" id="forgotpwdForm"
			method="post" class="parent">
			<p>
				<label for="usercode" class="label_input">&nbsp;账&nbsp;&nbsp;号:&nbsp;</label>
				<input type="text" id="usercode" placeholder="请输入账号" name="usercode"
			...
			<p>
				<label for="inputPassword" class="label_input">确认密码:</label> <input
					type="password" id="ps2" placeholder="请再次输入密码" class="custom-input"
					disabled>
			</p>
			...
			<button class="login_btn" type=submit>找回</button>

			<div>
				<p>
					<a onclick="login()">想起账号?返回登录</a>
				</p>
			</div>
   ...
   $(document).ready(function() {
	   ...
	   function startCountdown() {
		   ...
		   $("#validateButton").text(countdown + "秒再重试");
	...
	$(document).ready(function() {
	    $("#forgotpwdForm").submit(function(event) {
		    ...
		    if(ps1!=ps2){
		    	window.alert("密码不一致!");
		        return false;
		    }else if (usercode=="" || username=="" || ps1=="" || ps2==""){
		    	if (usercode==""){
			    	window.alert("ID不能为空！");
			        return false;
		    	}else if (username ==""){
